...
function renderPipelineDom(s,acc,anim,t,z){
  ...
  c.appendChild(el('div',`font-size:${px(30,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(5,z)};${as(anim,0,z)}`,escHtml(s.title||'')));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(14,z)};${as(anim,80,z)}`,escHtml(s.subtitle)));
  if(s.note) c.appendChild(el('div',`font-size:${px(12,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;margin-bottom:${px(12,z)};line-height:1.55;${as(anim,60,z)}`,escHtml(s.note)));
  ...
  steps.forEach((step,i)=>{
    ...
    lbl.style.cssText=`font-size:${px(12,z)};color:#d0d4e8;font-family:'JetBrains Mono',monospace;text-align:center;font-weight:600;letter-spacing:.02em;max-width:${nSz+20}px;line-height:1.3;`;
  ...
  if(s.callout){
    const ins=el('div','');
    ins.style.cssText=`border-radius:${px(8,z)};border:1px solid ${acc}40;background:${acc}0b;padding:${px(10,z)} ${px(16,z)};margin-top:${px(8,z)};font-size:${px(12,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;line-height:1.5;${as(anim,steps.length*140+150,z)}`;
...
function renderHeroSplitDom(s,acc,anim,t,z){
  ...
  if(s.title) content.appendChild(el('div',`font-size:${px(32,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(10,z)};${as(anim,0,z)}`,escHtml(s.title)));
  if(s.subtitle) content.appendChild(el('div',`font-size:${px(14,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(14,z)};${as(anim,60,z)}`,escHtml(s.subtitle)));
  ...
  (s.bullets||[]).forEach((b,i)=>{
    ...
    row.appendChild(el('span',`font-size:${px(15,z)};color:#d0d4e8;font-family:'JetBrains Mono',monospace;line-height:1.4`,escHtml(b)));
    content.appendChild(row);
  });

  if(s.note) content.appendChild(el('div',`font-size:${px(11,z)};color:#5a5f80;font-family:'JetBrains Mono',monospace;margin-top:${px(8,z)};${as(anim,400,z)}`,escHtml(s.note)));
...
function renderTerminalDom(s,acc,anim,t,z){
  ...
  // Title row
  c.appendChild(el('div',`font-size:${px(28,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(4,z)};${as(anim,0,z)}`,escHtml(s.title||'Terminal')));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(14,z)};${as(anim,80,z)}`,escHtml(s.subtitle)));
  ...
  bar.appendChild(el('div',`flex:1;margin-left:${px(12,z)};font-size:${px(11,z)};color:#5a5f80;font-family:'JetBrains Mono',monospace`,escHtml(s.terminalTitle||'bash — java-app')));
  win.appendChild(bar);
  ...
  lines.forEach((line,i)=>{
    ...
    const txt=el('span',`color:${isError?'#f87c7c':isSuccess?'#7cf8a0':isOutput?'#9095b8':'#e8eaf6'}`,escHtml(line.trim()));
    row.appendChild(txt);
  ...
  if(s.note){
    const note=el('div',`margin-top:${px(10,z)};font-size:${px(11,z)};color:#5a5f80;font-family:'JetBrains Mono',monospace;${as(anim,lines.length*90+300,z)}`);
...
function renderOrbitDiagramDom(s,acc,anim,t,z){
  ...
  // Center node
  const cntr=el('div',`position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);padding:${px(14,z)} ${px(20,z)};border-radius:${px(12,z)};border:2px solid ${acc};background:${acc}22;backdrop-filter:blur(8px);font-size:${px(14,z)};font-weight:700;color:${acc};font-family:'JetBrains Mono',monospace;text-align:center;white-space:nowrap;animation:scaleIn .5s ease 0ms both;box-shadow:0 0 ${px(30,z)} ${acc}45,inset 0 0 ${px(20,z)} ${acc}12;z-index:2`,escHtml(s.title||''));
...
function renderGlitchTitleDom(s,acc,anim,t,z){
  ...
  // Top tag
  if(s.tag||s.subtitle){
    const tag=el('div',`font-size:${px(11,z)};color:${acc};font-family:'JetBrains Mono',monospace;letter-spacing:.14em;border:1px solid ${acc}60;border-radius:${px(18,z)};padding:${px(5,z)} ${px(16,z)};margin-bottom:${px(20,z)};${as(anim,0,z)};animation:borderGlow 2.5s ease-in-out infinite;--gclr:${acc}`,escHtml(s.tag||s.subtitle||''));
    w.appendChild(tag);
  }

  // Main glitch title
  const titleWrap=el('div',`position:relative;text-align:center;${as(anim,120,z)}`);
  const mainTitle=el('div',`font-size:${px(72,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;line-height:1.1;text-align:center;letter-spacing:-.02em`);
  mainTitle.textContent=s.title||'';

  // Glitch clones
  const g1=el('div',`position:absolute;inset:0;font-size:${px(72,z)};font-weight:700;font-family:'JetBrains Mono',monospace;line-height:1.1;text-align:center;letter-spacing:-.02em;color:${acc};clip-path:inset(0 0 60% 0);transform:translateX(-3px);animation:glowPulse 2.5s ease-in-out .2s infinite;opacity:.35;pointer-events:none`);
  ...
  if(s.note||s.description){
    const desc=el('div',`font-size:${px(16,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;text-align:center;max-width:${px(620,z)};line-height:1.6;margin-top:${px(14,z)};${as(anim,380,z)}`,escHtml(s.note||s.description||''));
    w.appendChild(desc);
  }
  ...
  if(s.bullets&&s.bullets.length){
    ...
    s.bullets.forEach((b,i)=>{
      const chip=el('div',`padding:${px(6,z)} ${px(14,z)};border-radius:${px(16,z)};border:1px solid ${acc}40;background:${acc}10;font-size:${px(12,z)};color:${acc};font-family:'JetBrains Mono',monospace;animation:fadeUp .4s ease ${600+i*70}ms both`,'','bullet-item');
  ...
  // Bottom hint
  w.appendChild(el('div',`position:absolute;bottom:${px(24,z)};font-size:${px(11,z)};color:#2a2e45;font-family:'JetBrains Mono',monospace;${as(anim,800,z)}`,'▸ Press Space or → to continue'));
...
function renderBentoGridDom(s, acc, anim, t, z) {
  ...
  if (s.note || s.description) {
    w.appendChild(el('div', `font-size:${px(15,z)};color:#9095b8;margin-top:-${px(8,z)};${as(anim,100,z)}`, escHtml(s.note || s.description)));
  }
  ...
  bullets.forEach((b, i) => {
    ...
    const text = el('div', `font-size:${px(isLarge?18:14,z)};color:#e8eaf6;font-family:'Inter',sans-serif;font-weight:500;line-height:1.4`, escHtml(b));
...
function renderGlassFanDom(s, acc, anim, t, z) {
  ...
  bullets.forEach((b, i) => {
    ...
    card.appendChild(el('div', `font-size:${px(24,z)};font-weight:700;color:${colors[i%colors.length]};margin-bottom:${px(16,z)};font-family:'JetBrains Mono',monospace`, `0${i+1}`));
    card.appendChild(el('div', `font-size:${px(16,z)};color:#e8eaf6;line-height:1.5`, escHtml(b)));
...
function render3DCarouselDom(s, acc, anim, t, z) {
  ...
  bullets.forEach((b, i) => {
    ...
    const card = el('div', `position:absolute;width:${px(260,z)};height:${px(340,z)};background:${acc}0a;border:1px solid ${acc}40;border-radius:${px(16,z)};padding:${px(24,z)};display:flex;align-items:center;justify-content:center;text-align:center;font-size:${px(18,z)};color:#e8eaf6;backdrop-filter:blur(8px);transition:all 0.8s cubic-bezier(0.22,1,0.36,1) !important`, '', 'bullet-item carousel-card');
