...
function renderDiagramDom(s,acc,anim,t,z){
  ...
  c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(18,z)};${as(anim,80,z)}`,escHtml(s.subtitle||'')));
...
function buildCustomDiagram(nodes,acc,z){
  nodes=nodes||[];
  if(!nodes.length){
    return `<div style="font-size:${px(13,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace;text-align:center">Add boxes in the editor (Content tab → Diagram boxes)<br>to build your own diagram</div>`;
  }
  return `<div style="display:flex;gap:${px(16,z)};width:100%;flex-wrap:wrap;justify-content:center;align-items:stretch">
    ${nodes.map((nd,i)=>{
      ...
      const items=(nd.items||[]).map(it=>`<div style="padding:${px(6,z)} ${px(10,z)};margin-bottom:${px(5,z)};border-radius:${px(6,z)};border:1px solid rgba(255,255,255,0.05);background:rgba(0,0,0,0.25);font-size:${px(11,z)};color:#a5a9c4;font-family:'JetBrains Mono',monospace;display:flex;align-items:center;gap:${px(6,z)}"><div style="width:${px(4,z)};height:${px(4,z)};border-radius:50%;background:${c};box-shadow:0 0 6px ${c}"></div>${escHtml(it)}</div>`).join('');
...
function buildChainDiagram(nodes,acc,z){
  nodes=nodes||[];
  if(!nodes.length) return `<div style="font-size:${px(13,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace;text-align:center">Add steps in the editor</div>`;
  return `<div style="display:flex;flex-direction:column;align-items:center;gap:${px(8,z)};width:100%">${nodes.map((nd,i)=>{
    const c=nd.color||acc;
    const desc=(nd.items&&nd.items[0])?`<div style="font-size:${px(11,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;margin-top:${px(2,z)}">${escHtml(nd.items[0])}</div>`:'';
    return `<div style="width:${px(520,z)};padding:${px(12,z)} ${px(16,z)};border-radius:${px(10,z)};border:1px solid ${c}40;background:linear-gradient(90deg,${c}1a,transparent);box-shadow:0 ${px(6,z)} ${px(16,z)} rgba(0,0,0,0.3), inset 0 1px 1px rgba(255,255,255,0.05);backdrop-filter:blur(${px(10,z)});display:flex;align-items:center;gap:${px(16,z)};animation:slideIn .5s cubic-bezier(.22,1,.36,1) ${i*80}ms both">
      <div style="width:${px(28,z)};height:${px(28,z)};border-radius:50%;background:${c}20;border:1px solid ${c}60;display:flex;align-items:center;justify-content:center;font-size:${px(11,z)};color:${c};font-family:'JetBrains Mono',monospace;font-weight:700;box-shadow:0 0 ${px(12,z)} ${c}40;flex-shrink:0">${String(i+1).padStart(2,'0')}</div>
      <div><div style="font-size:${px(14,z)};color:#e8eaf6;font-family:'JetBrains Mono',monospace;font-weight:600;text-shadow:0 0 ${px(8,z)} ${c}30">${escHtml(nd.label||'')}</div>${desc}</div>
...
function buildColumnsDiagram(nodes,acc,z){
  nodes=nodes||[];
  if(!nodes.length) return `<div style="font-size:${px(13,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace;text-align:center">Add boxes in the editor</div>`;
  const cols=[[],[],[]];
  ...
  return `<div style="display:flex;gap:${px(32,z)};align-items:flex-start;justify-content:center">${cols.map(col=>{
    ...
    return `<div style="display:flex;flex-direction:column;align-items:center;gap:${px(10,z)}">${col.map((nd,i)=>{
      const c=nd.color||acc;
      const desc=(nd.items&&nd.items[0])?`<div style="font-size:${px(10,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;max-width:${px(160,z)};text-align:center;margin-top:${px(4,z)}">${escHtml(nd.items[0])}</div>`:'';
      return `<div style="display:flex;flex-direction:column;align-items:center;gap:${px(2,z)};animation:scaleIn .58s cubic-bezier(.22,1,.36,1) ${i*100}ms both">
        <div style="min-width:${px(160,z)};padding:${px(12,z)} ${px(16,z)};border-radius:${px(10,z)};border:1px solid ${c}50;background:linear-gradient(180deg,${c}1a,transparent);box-shadow:0 ${px(8,z)} ${px(20,z)} rgba(0,0,0,0.35), inset 0 1px 1px rgba(255,255,255,0.08);backdrop-filter:blur(${px(8,z)});display:flex;align-items:center;justify-content:center;font-size:${px(13,z)};font-family:'JetBrains Mono',monospace;color:${c};font-weight:700;text-align:center;text-shadow:0 0 ${px(12,z)} ${c}60">${escHtml(nd.label||'')}</div>
        ${desc}
      </div>`;
    }).join('')}</div>`;
  }).join('')}</div>`;
...
function buildLayeredDiagram(nodes,acc,z){
  nodes=nodes||[];
  if(!nodes.length) return `<div style="font-size:${px(13,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace;text-align:center">Add layers in the editor</div>`;
  return `<div style="display:flex;flex-direction:column;align-items:center;gap:0;width:100%;max-width:${px(640,z)};margin:0 auto">
    ${nodes.map((nd,i)=>{
      const c=nd.color||acc;
      const desc=(nd.items&&nd.items[0])?`<div style="font-size:${px(11,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;margin-top:${px(4,z)}">${escHtml(nd.items[0])}</div>`:'';
      return `<div style="width:100%;padding:${px(14,z)} ${px(22,z)};border-radius:${px(10,z)};border:1px solid ${c}45;background:linear-gradient(180deg,${c}1a,transparent);box-shadow:0 ${px(10,z)} ${px(30,z)} rgba(0,0,0,0.4), inset 0 1px 1px rgba(255,255,255,0.08);backdrop-filter:blur(${px(12,z)});animation:fadeUp .5s cubic-bezier(.22,1,.36,1) ${i*90}ms both">
        <div style="display:flex;align-items:center;gap:${px(10,z)}"><div style="width:${px(8,z)};height:${px(8,z)};border-radius:50%;background:${c};box-shadow:0 0 ${px(12,z)} ${c}"></div><div style="font-size:${px(15,z)};font-weight:700;color:${c};font-family:'JetBrains Mono',monospace;text-shadow:0 0 ${px(12,z)} ${c}60">${escHtml(nd.label||'')}</div></div>${desc}
...
function renderImageTextDom(s,acc,anim,t,z){
  ...
  hdr.appendChild(el('div',`font-size:${px(28,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(4,z)}`,escHtml(s.title||'')));
  ...
  (s.bullets||[]).forEach((b,i)=>{
    ...
    row.dataset.idx=i;
    row.appendChild(el('span',`color:${acc};font-size:${px(14,z)};flex-shrink:0;font-family:'JetBrains Mono',monospace`,'▸'));
    row.appendChild(el('span',`font-size:${px(15,z)};color:#d0d4e8;font-family:'JetBrains Mono',monospace;line-height:1.5`,escHtml(b)));
  ...
  if(s.imageUrl){
    ...
    if(s.imageCaption) imgSide.appendChild(el('div',`font-size:${px(11,z)};color:#5a5f80;font-family:'JetBrains Mono',monospace;margin-top:${px(8,z)};text-align:center`,escHtml(s.imageCaption)));
  } else {
    ...
    ph.appendChild(el('div',`font-size:${px(12,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace`,'Paste image URL in editor'));
...
function renderConceptMapDom(s,acc,anim,t,z){
  const c=el('div',`position:absolute;inset:0;padding:${px(36,z)} ${px(46,z)};display:flex;flex-direction:column`);
  c.appendChild(el('div',`font-size:${px(28,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(4,z)};${as(anim,0,z)}`,escHtml(s.title||'')));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(16,z)};${as(anim,80,z)}`,escHtml(s.subtitle)));
  const nodes=s.bullets&&s.bullets.length?s.bullets:(s.nodes||[]);
  if(!nodes.length){
    const hint=el('div',`flex:1;display:flex;align-items:center;justify-content:center;color:#3a3d52;font-family:'JetBrains Mono',monospace;font-size:${px(13,z)}`,'Add nodes in the editor (each bullet = a node)');
  ...
  const cntr=el('div',`position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);padding:${px(14,z)} ${px(22,z)};border-radius:${px(10,z)};border:2px solid ${acc};background:${acc}18;font-size:${px(14,z)};font-weight:700;color:${acc};font-family:'JetBrains Mono',monospace;text-align:center;white-space:nowrap;animation:scaleIn .5s ease 0ms both`,escHtml(s.title||''));
...
function renderBarChartDom(s,acc,anim,t,z){
  ...
  c.appendChild(el('div',`font-size:${px(28,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;margin-bottom:${px(2,z)};${as(anim,0,z)}`,escHtml(s.title||'')));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;margin-bottom:${px(22,z)};${as(anim,60,z)}`,escHtml(s.subtitle)));
  ...
  data.forEach((d,i)=>{
    ...
    colWrap.appendChild(el('div',`font-size:${px(13,z)};font-weight:700;color:${clr};font-family:'JetBrains Mono',monospace;margin-bottom:${px(6,z)}`,escHtml(String(d.value))+(s.chartUnit||'')));
    ...
    colWrap.appendChild(el('div',`font-size:${px(11,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;margin-top:${px(8,z)};text-align:center`,escHtml(d.label||'')));
...
function renderVennDom(s,acc,anim,t,z){
  ...
  if(s.title) c.appendChild(el('div',`font-size:${px(24,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(4,z)};${as(anim,0,z)}`,escHtml(s.title)));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(12,z)};color:${acc};font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(8,z)};${as(anim,60,z)}`,escHtml(s.subtitle)));
  ...
  const lbl=(text,leftPct,clr)=>el('div',`position:absolute;left:${leftPct}%;top:8%;transform:translateX(-50%);font-size:${px(15,z)};font-weight:700;color:${clr};font-family:'JetBrains Mono',monospace;background:#0b0d14;padding:${px(2,z)} ${px(10,z)};border-radius:${px(12,z)};border:1px solid ${clr}50`,escHtml(text));
  ...
  const listAt=(items,leftPct,w,clr,startIdx)=>{
    ...
    (items||[]).forEach((it,i)=>{
      const r=el('div',`font-size:${px(11,z)};color:${clr?'#0b0d14':'#d0d4e8'};background:${clr?clr+'cc':'transparent'};padding:${clr?px(3,z)+' '+px(8,z):'0'};border-radius:${px(10,z)};font-family:'JetBrains Mono',monospace;text-align:center;font-weight:${clr?'700':'400'}`,'','bullet-item');
...
function renderStackVisualDom(s,acc,anim,t,z){
  ...
  (s.bullets||[]).forEach((b,i)=>{
    const box=el('div',`width:${px(220,z)};padding:${px(12,z)};border-radius:${px(7,z)};border:1px solid ${acc}60;background:${acc}14;font-size:${px(14,z)};color:#e8eaf6;font-family:'JetBrains Mono',monospace;text-align:center`,'','bullet-item');
    box.dataset.idx=i; box.textContent=b;
    stackWrap.appendChild(box);
  });
  c.appendChild(stackWrap);
  c.appendChild(el('div',`font-size:${px(11,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace;margin-top:${px(10,z)}`,'▭ base of stack'));
  t.appendChild(c);
}
...
function renderProcessLoopDom(s,acc,anim,t,z){
  ...
  c.appendChild(el('div',`font-size:${px(26,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(4,z)};${as(anim,0,z)}`,escHtml(s.title||'')));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(12,z)};${as(anim,60,z)}`,escHtml(s.subtitle)));
  ...
  steps.forEach((st,i)=>{
    ...
    card.appendChild(el('div',`font-size:${px(11,z)};color:#d0d4e8;font-family:'JetBrains Mono',monospace;white-space:nowrap`,escHtml(st.label||'')));
...
function renderSpectrumDom(s,acc,anim,t,z){
  ...
  if(s.spectrumLabel){
    const lbl=el('div',`position:absolute;top:${px(-32,z)};left:${pos}%;transform:translateX(-50%);font-size:${px(12,z)};color:${acc};font-family:'JetBrains Mono',monospace;white-space:nowrap;font-weight:700`,escHtml(s.spectrumLabel));
    track.appendChild(lbl);
  }
  w.appendChild(track);
  const labels=el('div',`width:${px(600,z)};display:flex;justify-content:space-between;font-size:${px(13,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;${as(anim,250,z)}`);
...
function renderIconGridDom(s,acc,anim,t,z){
  const c=el('div',`position:absolute;inset:0;padding:${px(40,z)} ${px(54,z)};display:flex;flex-direction:column`);
  if(s.title) c.appendChild(el('div',`font-size:${px(28,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(4,z)};${as(anim,0,z)}`,escHtml(s.title)));
  if(s.subtitle) c.appendChild(el('div',`font-size:${px(13,z)};color:${acc};font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:${px(18,z)};${as(anim,60,z)}`,escHtml(s.subtitle)));
  ...
  items.forEach((it,i)=>{
    ...
    card.appendChild(el('div',`font-size:${px(13,z)};font-weight:700;color:#e8eaf6;font-family:'JetBrains Mono',monospace;text-align:center`,escHtml(it.label||'')));
    if(it.desc) card.appendChild(el('div',`font-size:${px(10,z)};color:#9095b8;font-family:'JetBrains Mono',monospace;text-align:center`,escHtml(it.desc)));
...
function renderImageFullDom(s,acc,anim,t,z){
  ...
  if(s.imageUrl){
    ...
    w.appendChild(el('div',`display:flex;flex-direction:column;align-items:center;gap:${px(10,z)};color:#3a3d52;font-family:'JetBrains Mono',monospace`,`<div style="font-size:${px(50,z)}">🖼️</div><div style="font-size:${px(13,z)}">Upload or paste an image URL in the editor</div>`));
  }
  if(s.title||s.imageCaption){
    ...
    if(s.title) overlay.appendChild(el('div',`font-size:${px(20,z)};font-weight:700;color:#fff;font-family:'JetBrains Mono',monospace`,escHtml(s.title)));
    if(s.imageCaption) overlay.appendChild(el('div',`font-size:${px(12,z)};color:${acc};font-family:'JetBrains Mono',monospace`,escHtml(s.imageCaption)));
