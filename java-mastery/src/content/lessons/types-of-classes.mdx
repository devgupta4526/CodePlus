...
```mermaid
flowchart TD
    ClassTypes((Java Class Types))
    ClassTypes --> Concrete["Concrete Class"]
    ClassTypes --> Abstract["Abstract Class"]
    ClassTypes --> SuperSub["Super / Sub Class"]
    ClassTypes --> Object["Object Class"]
    
    ClassTypes --> Nested["Nested Classes"]
    Nested --> Static["Static Nested Class"]
    Nested --> Inner["Non-Static / Inner Class"]
    Inner --> Member["Member Inner Class"]
    Inner --> Local["Local Inner Class"]
    Inner --> Anonymous["Anonymous Inner Class"]
    
    ClassTypes --> Generic["Generic Class"]
    ClassTypes --> POJO
    ClassTypes --> Enum
    ClassTypes --> Final["Final Class"]
    ClassTypes --> Singleton["Singleton Class"]
    ClassTypes --> Immutable["Immutable Class"]
    ClassTypes --> Wrapper["Wrapper Class"]
```
