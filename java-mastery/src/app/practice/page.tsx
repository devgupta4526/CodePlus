...
function ProblemsPanel({
  ...
  // Resizable split: descWidthPct is the percentage of the right two-column area
  // that the description panel occupies. Default ~42%.
  ...
  return (
    <div className="flex flex-1 max-w-[1400px] w-full mx-auto overflow-hidden" style={{ height: 'calc(100vh - 7.5rem)' }}>
      ...
      <div className="hidden md:flex flex-col w-[280px] xl:w-[300px] shrink-0 border-r border-[var(--border-color)] bg-[var(--bg-secondary)] overflow-hidden">
        ...
        {/* Footer stats */}
        <div className="px-4 py-3 border-t border-[var(--border-color)] flex items-center justify-between text-xs text-[var(--text-disabled)]">
      ...
      <div ref={containerRef} className="flex flex-1 min-w-0 overflow-hidden">
        ...
        <div
          className="flex flex-col border-r border-[var(--border-color)] overflow-hidden shrink-0"
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          {/* Editor toolbar */}
          <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-[var(--border-color)] bg-[var(--surface)] shrink-0">
            {/* Language picker */}
          ...
          <div className="flex-1 overflow-hidden min-h-0">
            <MonacoEditor
              ...
              value={showSolution ? selectedProblem.solutionCode[activeLang] : editorCode}
              onChange={showSolution ? undefined : onEditorChange}
              theme="vs-dark"
              options={{
          ...
          {/* Note if solution showing */}
          {showSolution && (
            <div className="px-4 py-2 bg-[var(--success)]/5 border-t border-[var(--success)]/20 text-xs text-[var(--success)] flex items-center gap-2">
              <CheckCircle2 className="w-3.5 h-3.5" />
              Showing reference solution for {LANGUAGES.find((l) => l.id === activeLang)?.label}
            </div>
          )}
        </div>
      </div>
...
function PlaygroundPanel({ challenges, selectedChallengeId, selectedChallenge, editorCode, execOutput, execError, onChallengeChange, onEditorChange, onRun, onReset }: {
  ...
  return (
    <div className="flex max-w-[1400px] mx-auto gap-5 p-5 h-[calc(100vh-7.5rem)]">
      {/* Challenge list */}
      <div className="w-64 shrink-0 space-y-2">
        <p className="text-xs font-bold text-[var(--text-disabled)] uppercase tracking-wider mb-3">JS Challenges</p>
        {challenges.map((c) => (
          <button
            key={c.id}
          ...
          >
            <div className="flex items-center justify-between gap-2 mb-1">
              <span className="text-sm font-semibold text-[var(--text-primary)] truncate">{c.title}</span>
              <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium border ${difficultyColors[c.difficulty]}`}>{c.difficulty}</span>
            </div>
            <p className="text-xs text-[var(--text-muted)] leading-relaxed">{c.description}</p>
          </button>
        ))}
        <div className="pt-3 border-t border-[var(--border-color)] text-xs text-[var(--text-disabled)]">
          JavaScript only. Runs in your browser.
        </div>
      </div>

      {/* Editor + output */}
      <div className="flex-1 flex flex-col rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--border-color)]">
          <div>
            <p className="text-sm font-semibold text-[var(--text-primary)]">{selectedChallenge.title}</p>
            <p className="text-xs text-[var(--text-muted)]">{selectedChallenge.description}</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={onReset} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[var(--border-color)] text-xs text-[var(--text-muted)] hover:bg-[var(--surface-elevated)] cursor-pointer transition-colors">
              <RotateCcw className="w-3.5 h-3.5" /> Reset
            </button>
            <button onClick={onRun} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-[var(--accent)] text-white text-xs font-medium hover:bg-[var(--accent-hover)] cursor-pointer transition-colors">
        ...
        {/* Output */}
        <div className="border-t border-[var(--border-color)] p-4 shrink-0">
          <div className="flex items-center gap-2 text-xs font-medium text-[var(--text-muted)] mb-2">
            <Terminal className="w-3.5 h-3.5 text-[var(--accent-secondary)]" /> Output
          </div>
          <div className={`rounded-xl border p-3 font-mono text-xs whitespace-pre-wrap min-h-16 max-h-28 overflow-y-auto ${execError ? 'border-[var(--error)]/30 text-[var(--error)]' : 'border-[var(--border-color)] text-[var(--text-secondary)]'} bg-[var(--bg-secondary)]`}>
...
function McqPanel({ questions, selectedAnswers, submittedAnswers, correctCount, answeredCount, onSelectAnswer, onSubmitAnswer }: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      {/* Score banner */}
      <div className="flex items-center justify-between p-4 rounded-2xl border border-[var(--border-color)] bg-[var(--surface)]">
        <div className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
          <ListChecks className="w-4 h-4" />
          <span><span className="font-semibold text-[var(--text-primary)]">{answeredCount}</span> of {questions.length} answered</span>
        </div>
        <div className="text-sm">
          Score: <span className="font-bold text-[var(--text-primary)]">{correctCount}</span><span className="text-[var(--text-muted)]">/{questions.length}</span>
        </div>
      </div>
          ...
          <div key={q.id} className="rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-[var(--border-color)]">
              <div className="flex items-center gap-3 flex-wrap mb-2">
                <span className="text-xs font-bold uppercase tracking-wider text-[var(--accent)]">Q{idx + 1}</span>
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${difficultyColors[q.difficulty]}`}>{q.difficulty}</span>
                <span className="text-xs px-2 py-0.5 rounded-full border border-[var(--border-color)] bg-[var(--surface-elevated)] text-[var(--text-muted)]">{q.topic}</span>
                {submitted && (
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-colors cursor-pointer disabled:cursor-default ${
                      showCorrect ? 'border-[var(--success)]/40 bg-[var(--success)]/10'
              ...
              <div className="pt-2">
                {!submitted ? (
                  <button
                    ...
                    disabled={!sel}
                    className="px-4 py-2 rounded-xl bg-[var(--accent)] text-white text-sm font-medium disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed hover:bg-[var(--accent-hover)] transition-colors"
                  >
                    Submit Answer
                  </button>
                ) : (
                  <div className="mt-1 rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4 text-sm text-[var(--text-secondary)] leading-relaxed">
